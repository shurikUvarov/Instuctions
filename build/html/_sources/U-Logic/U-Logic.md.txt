...
создавайте новые экземпляры. В U-Logic по умолчанию включены все необходимые узлы для работы с популярными протоколами и дополнительные вспомогательные узлы. Для каждого узла есть описание и примеры его использования. 
...
Чтобы развернуть все потоки в рабочей обалсти, или только потоки, содержащие измененные узлы, или только узлы, которые были изменены, или перезапустить развернутые потоки, нажмите на значок раскрытия кнопки развертывания. Выберите предпочитаемый вариант и затем нажмите кнопку развертывания соответственно.
...
Первая опция в меню позволяет выполнять основные действия по редактированию. Наведение на нее открывает дополнительные опции, перечисленные ниже:
...
Эта опция позволяет экспортировать созданный вами поток приложений. Она предоставляет две опции: "Скопировать в буфер обмена", которая позволяет копировать `flow.json`, и "Скачать", которая загружает `flow.json` файл.

![Export 1](../_static/U-Logic/Getting_Started/header/export.png)

Также в верхней части окна доступны опции для выбора области экспорта потока приложений:
...
![Flows 1](../_static/U-Logic/Getting_Started/header/conf_flows.png)
...
Эта опция перенаправит вас на официальный сайт UVATRON.
...
Рабочая область (пространство) — это основная область редактора, в которой вы создаете потоки приложений, перетаскивая узлы из палитры.
...
Вкладка «Внешний вид» предоставляет следующие возможности:
...
Также можно узнать и изменить метаданные в разделе "Module Properties"
...
<!-- Затем свойства отображаются как переменные среды внутри подпотока. -->
...
Узел — это фундаментальный блок, используемый для создания потоков. Каждый узел представляет собой отдельную часть функциональности или определенное действие, которое может быть выполнено в потоке.
...
Некоторые узлы также отображают свой статус выполнения с помощью значков в верхней части.
...
- Выберите нужный узел в диалоговом окне, которое содержит все доступные узлы из палитры узлов.
...
Узлы также можно добавлять, импортируя их из локальной библиотеки или используя буфер обмена (`главное меню` -> `импорт`).
...
###### Доступ к справочной информации узла
...
Вкладки, доступные на боковой панели, называются панелями.
...
Чтобы обновить контекстные переменные, нажмите кнопку обновления, расположенную в правом верхнем углу каждой вкладки области действия, или, чтобы обновить определенную переменную, наведите на нее курсор и нажмите кнопку обновления, которая появится справа от нее.

###### Удаление контекстных переменных

Чтобы удалить контекстную переменную, наведите указатель мыши на переменную, которую вы хотите удалить, и нажмите кнопку удаления, которая появится справа от нее.
...
В этом разделе будет рассмотрена внутренняя работа сообщений U-Logic, распространенные ошибки и лучшие практики для обеспечения плавного потока данных без ошибок.
...
![Пример использования узла trigger](../_static/U-Logic/Nodes/Function/trigger/trigger_scheme2.png)
...
- `delay`: Устанавливает задержку в миллисекундах, применяемую к сообщению. Эта опция применяется только в том случае, если узел настроен на разрешение сообщению переопределять настроенный интервал задержки по умолчанию. 
- `reset`: Если получено сообщение с этим свойством, любой текущий тайм-аут или повтор будут очищены, и сообщение не будет отправлено.
...
Узел U-Logic для выполнения базовых математических операций.

##### Входные параметры
- **Input field**: Имя поля во входном сообщении, содержащее число или массив чисел.
- **Output field**: Имя поля в выходном сообщении, в котором будет сохранён результат (как число или массив чисел).
- **Operation**: Доступны различные математические операции, которые могут быть выполнены над входными данными.

##### Доступные операции
- **Average (avg)** – среднее значение всех чисел в массиве.
- **Maximum (max)** – получение числа с наибольшим значением из массива.
- **Minimum (min)** – получение числа с наименьшим значением из массива.
- **Increment (inc)** – увеличение числа на 1.
- **Decrement (dec)** – уменьшение числа на 1.
- **Integer part (trunc)** – усечение числа до целой части.
- **Round upwards (ceil)** – округление числа вверх до ближайшего целого.
- **Round downwards (floor)** – округление числа вниз до ближайшего целого.
- **Nearest integer (round)** – округление числа до ближайшего целого.
- **Round decimal places (rdec)** – округление числа до указанного количества десятичных знаков.
- **Truncate decimal places (tdec)** – усечение числа до указанного количества десятичных знаков.
- **Sum (sum)** – сумма всех чисел в массиве.
- **Subtract (sub)** – вычитание всех чисел в массиве.
- **Multiply (mult)** – умножение всех чисел в массиве.
- **Divide (div)** – деление всех чисел в массиве.
- **Modulus (mod)** – получение остатка от деления двух чисел в массиве.
- **Absolute value (abs)** – абсолютное значение числа.
- **Random (rand)** – случайное число между 0 и 1.
- **Random between min and max (randb)** – случайное число между двумя числами в массиве.
- **Random from array (randa)** – случайное число из массива возможных чисел.
- **Length of array (len)** – длина массива.
- **Sort ascending (sorta)** – сортировка массива по возрастанию.
- **Sort descending (sortd)** – сортировка массива по убыванию.
- **Create range (range)** – создание массива целых чисел между двумя числами в массиве.
- **Get distance (dist)** – получение расстояния (диапазона) между числами в массиве.
- **X to the power of y (pow)** – возведение x в степень y (из массива двух чисел [x, y]).
- **E to the power of x (exp)** – значение E^x, где E — число Эйлера (примерно 2.7183).
- **Cubic root (cbrt)** – кубический корень числа.
- **Natural logarithm (log)** – натуральный логарифм числа по основанию E.
- **Logarithm (log10)** – логарифм числа по основанию 10.
- **Arccosine (acos)** – арккосинус числа.
- **Hyperbolic arccosine (acosh)** – гиперболический арккосинус числа.
- **Arcsine (asin)** – арксинус числа в радианах.
- **Hyperbolic arcsine (asinh)** – гиперболический арксинус числа.
- **Arctangent (atan)** – арктангенс числа в радианах.
- **Hyperbolic arctangent (atanh)** – гиперболический арктангенс числа.
- **Cosine (cos)** – косинус числа в радианах.
- **Hyperbolic cosine (cosh)** – гиперболический косинус числа.
- **Sine (sin)** – синус числа в радианах.
- **Hyperbolic sine (sinh)** – гиперболический синус числа.
- **Square root (sqrt)** – квадратный корень числа.
- **Tangent (tan)** – тангенс угла.
- **Hyperbolic tangent (tanh)** – гиперболический тангенс числа.

##### Дополнительные параметры
- Если операция не выбрана в конфигурации, её необходимо указать в поле `msg.operation`.
- Если операция требует одно число, его можно передать в `msg.payload`. Если передан массив, операция выполнится для всех элементов массива, а результат будет также массивом.

##### Constant
- В данном поле можно задать дополнительное число, которое будет автоматически добавлено в конец массива входных данных.

##### Round result to ... decimals
- Можно выбрать количество знаков после запятой для округления результата.

##### Truncate result to ... decimals
- Можно выбрать количество знаков после запятой для усечения результата.

#### string

Применяет методы строк к заданному свойству контекста и присваивает результаты, по желанию, тому же или другому свойству контекста. Методы могут быть объединены путем добавления нескольких методов последовательно с параметрами, которые могут быть литералами или значениями из других свойств контекста. Объект строки всегда передает объект `msg` с примененными изменениями (если таковые имеются).

По умолчанию методы `from` и `to` применяются к свойству `msg.payload`, но могут быть установлены для любого свойства глобального, потокового или контекста сообщения.

##### Подробности

```{note}
Этот узел ожидает, что заданное свойство контекста содержит строковые данные. Результат может содержать или не содержать строковые данные.
```

##### Методы, которые можно применить через интерфейс редактора узлов, и их поведение:

- **append(chars)** – добавляет указанную строку `chars` к строке. Например, `'Hello' append 'World'` становится `'HelloWorld'`.
- **between(left, right)** – извлекает строку между строками `left` и `right`. Например, `'The quick brown fox.' between 'quick' and 'fox'` становится `' brown '`.
- **camelize()** – удаляет любые подчеркивания или тире и преобразует строку в camelCase. Например, `'data_rate'` становится `'dataRate'`.
- **capitalize()** – делает первую букву строки заглавной. Например, `'jon'` становится `'Jon'`.
- **chompLeft(prefix)** – удаляет префикс из начала строки. Например, `'foobar' chompLeft 'foo'` становится `'bar'`.
- **chompRight(suffix)** – удаляет суффикс из конца строки. Например, `'foobar' chompRight 'bar'` становится `'foo'`.
- **collapseWhitespace()** – преобразует все смежные пробельные символы в один пробел.
- **contains(find)** – возвращает `true`, если строка содержит `find`. Например, `'JavaScript is one of the best languages.' contains 'one'` возвращает `true`.
- **count(find)** – возвращает количество вхождений `find`. Например, `'JP likes to program. JP does not play in the NBA.' count 'JP'` возвращает `2`.
- **dasherize()** – преобразует строку в camelCase в строку, разделенную тире. Например, `'dataRate'` становится `'data-rate'`.
- **decodeHTMLEntities()** – декодирует HTML-сущности в их строковое представление. Например, `'Ken Thompson &amp; Dennis Ritchie'` становится `'Ken Thompson & Dennis Ritchie'`.
- **decodeURIComponent()** – декодирует компонент URI. Например, `'http%3A%2F'` становится `'http://'`.
- **delLeftMost(find)** – возвращает оставшуюся строку после удаления первого вхождения `find` и всего, что находится слева от него. Например, `'\Parent\child\folders' delLeftMost '\child'` оставляет `'\folders'`.
- **delRightMost(find)** – возвращает оставшуюся строку после удаления первого вхождения `find` и всего, что находится справа от него. Например, `'\Parent\child\folders' delRightMost '\child'` оставляет `'\Parent'`.
- **encodeURIComponent()** – кодирует компонент URI. Например, `'http://'` становится `'http%3A%2F'`.
- **endsWith(chars)** – возвращает `true`, если строка заканчивается на `chars`. Например, `'hello jon' endsWith 'jon'` возвращает `true`.
- **escapeHTML()** – экранирует HTML. Например, `'<div>hi</div>'` становится `'&lt;div&gt;hi&lt;/div&gt;'`.
- **ensureLeft(prefix)** – гарантирует, что строка начинается с `prefix`. Например, `'subdir' ensureLeft '/'` становится `'/subdir'`.
- **ensureRight(suffix)** – гарантирует, что строка заканчивается на `suffix`. Например, `'dir' ensureRight '/'` становится `'dir/'`.
- **getRightMost(find)** – находит первое вхождение `find` и возвращает все, что находится справа от него (не включая `find`). Например, `'\Parent\child\folders' getRightMost '\child'` возвращает `'\folders'`.
- **getLeftMost(find)** – находит первое вхождение `find` и возвращает все, что находится слева от него (не включая `find`). Например, `'\Parent\child\folders' getLeftMost '\child'` возвращает `'\Parent'`.
- **humanize()** – преобразует ввод в удобочитаемую форму. Например, `' capitalize dash-CamelCase_underscore trim '` становится `'Capitalize dash camel case underscore trim'`.
- **isAlpha()** – возвращает `true`, если строка содержит только буквы.
- **isAlphaNumeric()** – возвращает `true`, если строка содержит только буквы и цифры.
- **isEmpty()** – возвращает `true`, если строка состоит только из пробелов или является `null/undefined`.
- **isLower()** – возвращает `true`, если символ или строка в нижнем регистре.
- **isNumeric()** – возвращает `true`, если строка содержит только цифры.
- **isUpper()** – возвращает `true`, если символ или строка в верхнем регистре.
- **latinise()** – удаляет акценты с латинских символов. Например, `'crème brûlée'` становится `'creme brulee'`.
- **left(n)** – возвращает подстроку, состоящую из `n` левых символов. Например, `'My name is JP' left 2` возвращает `'My'`.
- **length()** – возвращает длину строки. Например, длина `'hi'` равна `2`.
- **lines()** – возвращает массив строк. Поддерживает совместимость с различными платформами.
- **pad(length, char)** – дополняет строку до указанной длины символом `char` по центру. Например, `'hello' pad '.'` становится `'...hello..'`.
- **padLeft(length, char)** – дополняет строку слева. Например, `'hello' padLeft '.'` становится `'.....hello'`.
- **padRight(length, char)** – дополняет строку справа. Например, `'hello' padRight '.'` становится `'hello.....'`.
- **parseCSV()** – разбирает строку CSV в массив. Например, `"'a','b','c'" becomes ['a', 'b', 'c']`.
- **repeat(n)** – повторяет строку `n` раз. Например, `'*' repeat 5` становится `'*****'`.
- **replaceAll(search, replace)** – заменяет все вхождения `search` на `replace`. Например, `' does IT work? ' replaceAll ' ', '_'` становится `'_does_IT_work?_'`.
- **right(n)** – возвращает подстроку, состоящую из `n` правых символов. Например, `'My name is JP' right 2` возвращает `'JP'`.
- **setValue(chars)** – устанавливает содержимое в `chars`.
- **slugify()** – преобразует текст в допустимый URL-slug. Удаляет акценты с латинских символов. Например, `'The Quick Brown Fox'` становится `'the-quick-brown-fox'`.
- **split(separator, limit)** – разбивает строку на массив подстрок, используя `separator`. Можно указать лимит количества разбиений. Например, `'Hello planet earth' split ' '` возвращает `['Hello', 'planet', 'earth']`.
- **splitLeft(separator, maxSplit, limit)** – разбивает строку слева направо, выполняя не более `maxSplit` разбиений и ограничивая результат `limit` элементами. Например, `'We built this city' splitLeft ' ' and 1` возвращает `['We', 'built this city']`.
- **splitRight(separator, maxSplit, limit)** – разбивает строку справа налево, выполняя не более `maxSplit` разбиений и ограничивая результат `limit` элементами. Например, `'We built this city' splitLeft ' ' and 1` возвращает `['We built this', 'city']`.
- **startsWith(prefix)** – возвращает `true`, если строка начинается с `prefix`. Например, `'Steve was here' startsWith 'Steve'` возвращает `true`.
- **strip(chars)** – удаляет символы `chars` из строки. Например, `' Hello World ' strip ' '` становится `'HelloWorld'`.
- **stripPunctuation()** – удаляет все знаки препинания. Например, `'My, st[ring] *full* of %punctuation)'` становится `'My string full of punctuation'`.
- **stripTags(tag)** – удаляет все HTML-теги с указанным тегом или все теги, если тег не указан. Например, `'<p>just <b>some</b> text</p>' stripTags` становится `'just some text'`.
- **template(values, open, close)** – интерполирует значения в строку. По умолчанию используются `{{` и `}}` для Mustache-совместимых шаблонов, но их можно изменить. Например, `'Hello {{planet}}!!!' template {"planet":"Earth"}` становится `'Hello Earth!!!'`.
- **titleCase()** – изменяет строку так, чтобы первая буква каждого слова была заглавной. Например, `'hello world' titleCase` становится `'Hello World'`.
- **toBoolean()** – преобразует строку в логическое значение. Строки, содержащие `'true'`, `'on'`, `'yes'` или `'1'`, становятся `true`, иначе `false`.
- **toCSV(JSON)** – преобразует массив или объект в строку CSV.
- **toFloat(precision)** – разбирает строку и возвращает число с плавающей точкой с указанной точностью. Например, `'3.45522' toFloat 2` становится `3.45`.
- **toInteger()** – разбирает строку и возвращает целое число. Например, `'5.3' toInteger` становится `5`.
- **toLowerCase()** – преобразует строку в нижний регистр. Например, `'Hello world' toLowerCase` становится `'hello world'`.
- **toString()** – преобразует текущее значение в строку. Например, `123 toString` становится `'123'`.
- **toUpperCase()** – преобразует строку в верхний регистр. Например, `'Hello world' toUpperCase` становится `'HELLO WORLD'`.
- **trim()** – удаляет пробелы в начале и конце строки. Например, `' hello ' trim` становится `'hello'`.
- **trimLeft()** – удаляет пробелы в начале строки. Например, `' hello there.' trimLeft` становится `'hello there.'`.
- **trimRight()** – удаляет пробелы в конце строки. Например, `'hello there ' trimRight` становится `'hello there'`.
- **truncate(length, chars)** – усекает строку, учитывая размещение слов и количество символов. Например, `'this is some long text' truncate 14 and ' read more'` становится `'this is some read more'`.
- **underscore()** – преобразует строку в camelCase в строку, разделенную подчеркиваниями. Например, `'dataRate' underscore` становится `'data-rate'`.
- **unescapeHTML()** – раскрывает HTML-сущности. Например, `'&lt;div&gt;hi&lt;/div&gt;' unescapeHTML` становится `'<div>hi</div>'`.
- **wrapHTML(tag, attributes)** – оборачивает строку в HTML-элемент с указанными атрибутами. Например, `'Venkat' wrapHTML 'div' and { "class": "left bullet" }` становится `'<div id="content" class="left bullet">Venkat</div>'`.
...
- **Метод**: Выберите метод HTTP для запроса (GET, POST, PUT, DELETE, HEAD). Также возможно задать его динамически с помощью `msg.method`.
- **URL**: Укажите URL конечной точки для связи. Динамическая настройка URL разрешена с помощью `msg.url`. Кроме того, если вы хотите создать URL со свойством сообщения, вы можете использовать теги в стиле Mustache с двойными фигурными скобками `{{ }}`. Например, `example.com/{{topic}}`, где значение `msg.topic` будет вставлено автоматически. Использование двойных фигурных скобок `{{ }}` по умолчанию выполняет экранирование HTML, поэтому специальные символы в заменяемом значении будут экранированы. Однако, если вы хотите сохранить специальные символы, такие как `/` и `&` в созданном URL, вы можете использовать тройные фигурные скобки `{{{ }}}`.
- **Данные**: позволяет выбрать, как полученные данные от предыдущего узла будут отправлены ​​вместе с запросом:
  - **Игнорировать**: если включено, данные будет игнорироваться.
  - **Добавлять к параметрам строки запроса**: включение этой опции позволит отправлять параметры строки запроса URL с помощью `msg.payload`.
  - **Отправлять как тело запроса**: отправить `payload` как часть тела запроса.
...
- **Заголовки**: позволяет добавлять заголовки к HTTP-запросу, например content-type, accept, user agent и т. д. Вы можете динамически устанавливать заголовки с помощью `msg.headers`. Однако сброс `msg.headers` позволяет избежать непреднамеренного наследования заголовков при использовании нескольких узлов http request в одном потоке. Более того, если `msg.payload` это объект, узел автоматически устанавливает Content-Type значение `application/json.

##### Вариант использования

1. **Интеграция API**: узел http request обеспечивает бесшовную интеграцию с внешними API. Разработчики могут использовать его для извлечения данных из API с помощью запросов GET или отправки данных в API с помощью запросов POST/PUT. Например, извлечение данных о погоде из API погоды или отправка данных в службу обмена сообщениями, являются распространенными сценариями.

2. **Удаленное управление и управление устройствами**: в системах умного дома узел http request может использоваться для упрощения удаленного управления устройствами. Он позволяет пользователям управлять различными устройствами, такими как освещение, термостаты и камеры безопасности, через веб-интерфейсы или мобильные интерфейсы, взаимодействуя с API устройств. Такие действия, как переключение устройств, настройка параметров и получение обновлений в реальном времени, могут быть выполнены через узел http request.
...
1. Ниже приведен пример, показывающий, как можно создать URL-адрес со свойствами сообщения в узле http request. Пример включает отправку запроса GET.

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example1.png)

2. Ниже приведен пример, показывающий, как можно отправить HTTP POST-запрос на фиктивный API. Этот пример включает регистрацию пользователя.

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example2_edit.png)

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example2.png)

3. Ниже приведен пример, демонстрирующий, как можно динамически задать URL, метод и заголовки для узла http request. Кроме того, этот пример иллюстрирует отправку запроса GET на фиктивный API с токеном авторизации.

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example3_edit.png)

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example3.png)

4. Ниже приведен пример, показывающий, как можно отправить запрос PUT с помощью узла http request.

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example4_edit.png)

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example4.png)

5. Ниже приведен пример, показывающий, как можно отправить запрос DELETE с помощью узла http request.

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example5_edit.png)

![Пример использования узла http request](../_static/U-Logic/Nodes/Network/http_request/example5.png)
...
- `url`: Если не настроено в узле, это необязательное свойство задает URL-адрес запроса.
- `method`: Если не настроено в узле, это необязательное свойство задает метод HTTP запроса. Должен быть одним из GET, PUT, DELETE или HEAD.
- `headers`: Устанавливает заголовки HTTP запроса. Любые заголовки, установленные в конфигурации узла, перезапишут любые соответствующие заголовки в `msg.headers`.
- `cookies`: Если установлено, может использоваться для отправки файлов cookie вместе с запросом.
- `payload`: Отправлено как текст запроса.
- `rejectUnauthorized`: Если установлено значение `false`, разрешается отправлять запросы на https-сайты, использующие самоподписанные сертификаты.
- `followRedirects`: Если установлено в `false`, предотвращает следование перенаправлениям (HTTP 301). По умолчанию true.
- `requestTimeout`: Если задано положительное число миллисекунд, будет переопределен глобально заданный httpRequestTimeout параметр.

###### Выходные данные

- `payload`: Тело ответа. Узел можно настроить так, чтобы он возвращал тело как строку, пытался проанализировать его как строку JSON или оставлял его как двоичный буфер.
- `statusCode`: Код статуса ответа или код ошибки, если запрос не может быть выполнен.
- `headers`: Объект, содержащий заголовки ответа.
- `responseUrl`: В случае, если при обработке запроса произошли перенаправления, это свойство является окончательным перенаправленным URL. В противном случае — URL исходного запроса.
- `responseCookies`: Если ответ включает файлы cookie, это свойство представляет собой объект пар «имя/значение» для каждого файла cookie.
- `redirectList`: Если запрос был перенаправлен один или несколько раз, накопленная информация будет добавлена ​​в это свойство. `location` — следующий пункт назначения перенаправления. `cookies` — файлы cookie, возвращаемые источником перенаправления.
...
###### Отправить/Получить
- payload: эта опция отправляет или получает только данные `msg.payload` через соединение WebSocket. Она исключает любые дополнительные свойства `msg`.
- Полное сообщение: если этот параметр включен, он позволяет отправлять или получать весь объект сообщения, включая payload и другие свойства, в виде строки в формате JSON.
...
![Пример использования узлов websocket in и websocket out](../_static/U-Logic/Nodes/Network/WebSocket/websocket_example.png)
...
- payload: Поток или отдельное сообщение данных, которые вы отправляете/получаете:
  ...
  - cтрока Base64: данные отправляются/принимаются как строка Base64.
    - разделены с: Укажите разделитель для разделения входящих потоков данных. Укажите разделитель для разделения входящих потоков данных. Обычно, `,`, `\r`, `\n`.
...
1. В приведенном ниже примере мы создаем базовый TCP-сервер с использованием узла tcp in.

![Пример использования узла tcp in](../_static/U-Logic/Nodes/Network/tcpin/tcpin_example1.png)

2. В приведенном ниже примере потока мы демонстрируем, как использовать узел tcp in вместе с другими узлами TCP для обеспечения двунаправленной связи.

![Пример использования узла tcp in](../_static/U-Logic/Nodes/Network/tcpin/tcpin_example2.png)
...
- `msg.payload`: Вывод полученных сообщений в виде буфера, строки или строки в кодировке base64.
- `msg.fromip`: IP-адрес и порт, с которого было получено сообщение, в формате «IP:Port».
- `msg.ip` и `msg.port`: IP-адрес и порт, с которого было получено сообщение.
...
- Отправлять:
  - UDP сообщение: Укажите IP-адрес целевого устройства или службы. Вы также можете указать IP-адрес динамически с помощью `msg.ip`, если это поле оставить пустым.
  - Широковещательное сообщение: Отправка сообщения всем устройствам в сети. Вы можете указать адрес как локальный широковещательный IP-адрес или использовать 255.255.255.255, который представляет собой глобальный широковещательный адрес.
...
- на порт:
  - Укажите номер порта, на который будут отправляться пакеты UDP. Вы также можете указать IP-адрес динамически с помощью `msg.port`, если оставить это поле пустым.
- Адрес:
...
1. В приведенном ниже примере у нас есть узел udp in, настроенный на прием данных, отправленных через локальный хост и порт 90, с использованием узла udp out.

![Пример использования узлов udp in и udp out](../_static/U-Logic/Nodes/Network/udp/udp_example.png)
...
##### Узел ssh-v3 в U-Logic

Этот узел устанавливает SSH-соединение с удаленным сервером и выполняет команды на нем. Он настраивается с помощью параметров SSH из соответствующего узла конфигурации SSH (ssh-conf).

##### Входные значения
Этот узел принимает сообщения с следующими свойствами:

- `payload`: Команда для выполнения на удаленном SSH-сервере.
Выходы

##### Выходные значения
После выполнения команды на сервере этот узел отправляет сообщение с следующими свойствами:
- `payload`: Результат выполнения команды.
- `session`: Информация о сеансе выполнения, включая `code`, `stdout` и `stderr`.

##### Настройка

Name: Опционально. Название узла SSH-подключения.
Hostmae: IP адрес сервера.
Port: Порт для подключения.
Username: Имя пользователя.
Password: Пароль.
Использование msg.reset
Для сброса SSH-соединения и принудительного повторного подключения отправьте сообщение с `msg.reset` установленным в true. Это закроет текущее SSH-соединение и повторно подключится при следующем вводе команды.

Пример использования `msg.reset`:
```javascript
{
    "payload": "",
    "reset": true
...
```
```{note}
Убедитесь, что узел конфигурации SSH (ssh-conf) правильно настроен с корректными учетными данными (имя пользователя, пароль, хост и т. д.) перед использованием этого узла.

Команды, выполняемые на удаленном сервере, передаются через свойство `payload` входного сообщения.
```
...
Пингует устройство и возвращает время отклика в мс в `msg.payload`.

##### Выходные данные
- `payload` – Время отклика в мс или `false`, если нет ответа.
- `topic` – Целевой хост или IP-адрес.
- `ping` – Объект с информацией о хосте и дополнительными свойствами.

```{note}
Объект `ping` добавляется только в режиме (Mode) `Triggered`, если передан массив в `msg.payload`. Это позволяет включать дополнительные свойства для использования в последующих узлах.
```

##### Детали
Возвращает `false`, если хост не отвечает или его невозможно разрешить.

По умолчанию пинг выполняется каждые 20 секунд (настраивается).

###### Протокол
- Automatic– Выбирает IPv4 или IPv6 в зависимости от сетевых настроек системы.
- IPv4 – Принудительное использование IPv4 (не сработает, если нет маршрута).
- IPv6 – Принудительное использование IPv6 (не сработает, если нет маршрута).

###### Режимы работы
**Timed**
В этом режиме необходимо указать:
- Target – CSV-список хостов/IP, например `"192.168.0.1"` или `"192.168.0.1, www.google.com"`.
- Ping (S) – Интервал пинга в секундах.

**Triggered**
В этом режиме узел активируется входящим сообщением.

- Если Target указан, он будет использоваться как хост/IP.  
- Если Target пуст, можно передавать хосты через `msg.payload`:
  - Строка – CSV-список хостов/IP, например `"192.168.0.1"` или `"192.168.0.1, www.google.com"`.
  - Массив – Список хостов в виде строк или объектов.

##### Пример массива `payload`
```json
[
    "192.168.0.99",
    {
...
Простой узел для получения одного или нескольких SNMP OID. Активируется любым входным сигналом.

###### Входные параметры
- `msg.host` – Адрес целевого устройства.
- `msg.community` – SNMP-сообщество.
- `msg.username` – Имя пользователя (только для SNMP V3).
- `msg.authkey` – Ключ аутентификации (только для SNMP V3).
- `msg.privkey` – Ключ шифрования (только для SNMP V3).
- `msg.oid` – Список OID, разделённых запятыми (без пробелов).

OID должны быть числовыми. `iso.` эквивалентен `1`.

###### Выходные параметры
- `msg.payload` – Значение OID.
- `msg.oid` – Запрошенный OID.

##### snmp set

Простой узел для установки значений SNMP. Активируется любым входным сигналом.
...
- `msg.authkey` – Ключ аутентификации (только для SNMP V3).
- `msg.privkey` – Ключ шифрования (только для SNMP V3).
- `msg.varbinds` – Массив JSON-объектов, содержащий несколько OID, их типы и значения.
...
Числовые значения должны быть числами, а не строками, например `1`, а не `"1"`.

OID должны быть числовыми. `iso.` эквивалентен `1`.

##### snmp table

Простой узел для получения таблицы SNMP OID. Активируется любым входным сигналом.
...
- `msg.authkey` – Ключ аутентификации (только для SNMP V3).
- `msg.privkey` – Ключ шифрования (только для SNMP V3).
- `msg.oid` – OID таблицы, которую необходимо запросить.

OID должны быть числовыми. `iso.` эквивалентен `1`.

###### Выходные параметры
- `msg.payload` – Полученные данные.
- `msg.oid` – Запрошенный OID.

##### snmp subtree

Простой узел для получения поддерева SNMP OID. Активируется любым входным сигналом. Считывает все OID на текущем базовом уровне и ниже.
...
Отправляет Wake-On-LAN пакет magic на указанный MAC-адрес.

##### Входные параметры
- `msg.mac` – MAC-адрес целевого устройства, которое нужно разбудить (может быть задан динамически).
- `msg.host` – Адрес целевой машины (опционально).
- `msg.udpport` – Целевой UDP-порт (по умолчанию `9`).
- `msg.numpackets` – Количество отправляемых пакетов (по умолчанию `3`).
- `msg.interval` – Интервал между пакетами в миллисекундах (по умолчанию `100ms`).

##### Описание
Для наилучшей работы целевой адрес следует устанавливать в широковещательный адрес подсети, к которой подключено целевое устройство.  
Для сети класса C это, как правило, первые три части IP-адреса, а последняя часть — `255`, например: `192.168.1.255`.
...
1. Разбиение строки текста на серию сообщений, каждое из которых содержит одно слово текста.
2. Разбиение массива данных на серию сообщений, каждое из которых содержит один элемент массива.
3. Разделение объекта по ключам объекта для создания серии сообщений, каждое из которых содержит одну пару ключ-значение объекта.
4. Разделение сообщения по символу-разделителю для создания серии сообщений, каждое из которых содержит одну часть сообщения.
...
![Пример использования узла split](../_static/U-Logic/Nodes/Sequense/split/example1.png)
...
![Пример использования узла split](../_static/U-Logic/Nodes/Sequense/split/example2.png)
...
![Пример использования узла split](../_static/U-Logic/Nodes/Sequense/split/example3.png)
...
![Пример использования узла split](../_static/U-Logic/Nodes/Sequense/split/example4.png)
...
![Пример использования узла split](../_static/U-Logic/Nodes/Sequense/split/example5.png)
...
- `payload`: Поведение узла определяется типом `msg.payload`:
  - строка / буфер - сообщение разбивается с использованием указанного символа (по умолчанию: `\n`), последовательности буфера или на фиксированные длины.
...
- `parts`:
  - Это свойство содержит информацию о том, как сообщение было разделено с исходным сообщением. Если передать его в узел соединения , последовательность может быть повторно собрана в одно сообщение. Свойство имеет следующие свойства:
  - `id` - идентификатор группы сообщений
  - `index` - положение в группе
  - `count`- если известно, общее количество сообщений в группе.
  - `type` - тип сообщения - строка/массив/объект/буфер
  - `ch` - для строки или буфера данные, используемые для разделения сообщения как строки или массива байтов
  - `key` - для объекта, ключ свойства, из которого было создано это сообщение. Узел можно настроить на копирование этого значения в другие свойства сообщения, например `msg.topic`.
  - `len` - длина каждого сообщения при разделении с использованием фиксированного значения длины
...
- Сортировать:
  - `msg.<property>`: Укажите свойство сообщения, которое содержит данные массива и которое необходимо отсортировать.
  - последовательности сообщений: сортировка последовательности сообщений. Выбор этого узла сортировки зависит от полученных сообщений, которые должны быть установлены в `msg.parts`. Узел split генерирует это свойство, но его можно создать вручную. Он имеет следующие свойства:
    - id: Идентификатор группы сообщений.
    - index: Положение в группе.
...
1. В приведенном ниже примере узел sort сортирует массив чисел и массив английского алфавита в порядке возрастания.

![Пример использования узла sort](../_static/U-Logic/Nodes/Sequense/sort/example1.png)

2. В приведенном ниже примере узел sort сортирует последовательности сообщений в порядке возрастания на основе `msg.payload` свойств `quantity` каждого сообщения.

![Пример использования узла sort](../_static/U-Logic/Nodes/Sequense/sort/example2.png)
...
![Пример использования узла batch](../_static/U-Logic/Nodes/Sequense/batch/example1.png)

Кроме того, вы можете добавить перекрытие, чтобы последнее сообщение было включено в следующий пакет.

![Пример использования узла batch](../_static/U-Logic/Nodes/Sequense/batch/example2.png)
...
![Пример использования узла batch](../_static/U-Logic/Nodes/Sequense/batch/example3.png)
...
![Пример использования узла batch](../_static/U-Logic/Nodes/Sequense/batch/example4.png)

![Пример использования узла batch](../_static/U-Logic/Nodes/Sequense/batch/example5.png)
  ...
  - ручной: позволяет вручную выбирать, как следует объединять сообщения.
    - Объединить каждый: позволяет выбрать, какие свойства сообщения должны быть объединены. Дополнительно позволяет выбрать полный объект сообщения для объединения.
      ...
      - Отправить сообщение:
        ...
        - по истечении времени с 1го сообщения: Ожидает тайм-аут после получения первой части сообщения. Если в течение указанного тайм-аута прибудет больше частей, они будут включены в объединенное сообщение. Тайм-аут можно перезапустить, отправив сообщение с установленным свойством `msg.restartTimeout`.
        - после сообщения с установленным свойством `msg.complete`: Если получено сообщение с `msg.complete` установленным свойством, выходное сообщение завершается и отправляется. Это сбрасывает все счетчики частей. Кроме того, если получено сообщение с установленным свойством `msg.reset`, частично завершенное сообщение удаляется и не отправляется, что также сбрасывает все счетчики частей.
...
1. В приведенном ниже примере узел join использует автоматический режим для повторного присоединения сообщения, разделенного узлом split.

![Пример использования узла join](../_static/U-Logic/Nodes/Sequense/join/example1.png)

2. В приведенном ниже примере узел Join использует ручной режим для объединения трех свойств полезной нагрузки в одно на основе значения объекта `msg.topic`.

![Пример использования узла join](../_static/U-Logic/Nodes/Sequense/join/example2.png)

3. В приведенном ниже примере узел Join вычисляет размер массива, использующий агрегирующее выражение.

![Пример использования узла join](../_static/U-Logic/Nodes/Sequense/join/example3.png)
...
- `parts`: Для автоматического присоединения последовательности сообщений, все они должны иметь это свойство. Узел split генерирует это свойство, но его можно создать вручную. Он имеет следующие свойства:
  - `id` - идентификатор группы сообщений
  - `index` - положение в группе
  - `count` - общее количество сообщений в группе
  - `type` - тип сообщения - строка/массив/объект/буфер
  - `ch` - для строки или буфера данные, используемые для разделения сообщения как строки или массива байтов
  - `key` - для объекта — ключ свойства, из которого было создано это сообщение
  - `len` - длина каждого сообщения при разделении с использованием фиксированного значения длины
- `complete`: Если установлено, узел добавит payload, а затем отправит выходное сообщение в его текущем состоянии. Если вы не хотите добавлять payload, удалите ее из сообщения.
- `reset`: Если установлено, узел очистит любое частично завершенное сообщение и не отправит его.
- `restartTimeout`: Если установлено и на узле настроен тайм-аут, этот тайм-аут будет перезапущен.
...
строка или буфер — создаются путем объединения выбранного свойства каждого сообщения с указанными символами объединения или буфером.
...
Остальные свойства выходного сообщения берутся из последнего сообщения, полученного перед отправкой результата.
...
При настройке на объединение в режиме сокращения выражение применяется к каждому сообщению в последовательности, а результат накапливается для создания одного сообщения.

- Начальное значение: Начальное значение накопленной величины (`$A`).
- Агрегирующее выражение: Выражение JSONata, которое вызывается для каждого сообщения в последовательности. Результат передается следующему вызову выражения как накопленное значение. В выражении могут использоваться следующие специальные переменные:
...
##### Для чего используется узел csv в U-Logic?

Узел csv в U-Logic используется для обработки данных, разделенных запятыми (CSV). Он позволяет либо преобразовать строку CSV в объект JavaScript, либо преобразовать объект в строку CSV. Эта возможность делает его чрезвычайно полезным для чтения и записи данных в табличном формате.

##### Пример использования

Предположим, у вас есть CSV-файл, в котором подробно описывается потребление энергии различными производственными станциями на заводе. Файл содержит название станции, месяц и общее потребление электроэнергии. Вы хотите добавить новый столбец, в котором отображается количество деталей, произведенных на станции, исходя из предположения, что каждая деталь потребляет определенное количество электроэнергии в дополнение к базовому потреблению электроэнергии станком.
...
![Пример использования узла csv](../_static/U-Logic/Nodes/Parsers/csv/csv_example.png)

##### Документация по узлу

Выполняет преобразование между строкой в ​​формате CSV и ее представлением объекта JavaScript в любом направлении.

###### Входные значения

- `payload`: Объект JavaScript, массив или строка CSV.

###### Выходные значения

- `payload`:
  - Если входные данные являются строкой, он пытается проанализировать ее как CSV и создает объект JavaScript пар ключ/значение для каждой строки. Затем узел либо отправит сообщение для каждой строки, либо одно сообщение, содержащее массив объектов.
  - Если входные данные представляют собой объект JavaScript, он пытается создать строку CSV.
  - Если входные данные представляют собой массив простых значений, создается однострочная CSV-строка.
  - Если входные данные представляют собой массив массивов или массив объектов, создается многострочная CSV-строка.

###### Подробности

Шаблон столбца может содержать упорядоченный список имен столбцов. При конвертации CSV в объект имена столбцов будут использоваться как имена свойств. В качестве альтернативы имена столбцов могут быть взяты из первой строки CSV.

Если отмечена опция «разбирать числовые значения», строковые числовые значения будут возвращены как числа, т. е. среднее значение «1,"1.5",2».

Если отмечен параметр «включать пустые строковые значения», в результате будут возвращены пустые строки, т. е. среднее значение '"1","",3'.

Если отмечена опция «Включить нулевые значения», в результате будут возвращены нулевые значения, т. е. среднее значение '"1","","3"'.

Узел может принимать многокомпонентные входные данные, если partsсвойство задано правильно, например, из узла ввода файла или узла разделения.

При выводе нескольких сообщений для них будут установлены свои свойства `parts`, и они сформируют полную последовательность сообщений.

Если узел настроен на отправку заголовков столбцов только один раз, то установка любого значения `msg.reset` приведет к повторной отправке заголовков узлом.

#### html

##### Документация по узлу

Извлекает элементы из HTML-документа, хранящегося в `msg.payload` с помощью селектора CSS.

###### Входные значения

- `payload`: HTML-строка, из которой извлекаются элементы.
- `select`: Если селектор не настроен на панели редактирования, его можно задать как свойство сообщения.

###### Выходные значения

- `payload`: результатом может быть либо одно сообщение с payload, содержащей массив совпавших элементов, либо несколько сообщений, каждое из которых содержит совпавший элемент. Если отправлено несколько сообщений, они также будут иметь набор `parts`.

#### json

##### Для чего используется узел JSON в U-Logic?

Узел JSON в U-Logic используется для обработки данных JavaScript Object Notation (JSON). Он позволяет выполнять преобразование между строкой в ​​формате JSON и объектом JavaScript, что делает его весьма универсальным для задач, связанных с манипуляцией JSON. Эта возможность преобразования оказывается существенной при работе с API, хранении данных или передаче данных между различными службами.

##### Пример использования

Предположим, у вас есть поток данных JSON, который поступает от различных датчиков, установленных на сборочной линии на производственном предприятии. Объекты JSON включают такие данные, как название оборудования, временная метка и процент эффективности. Вы хотели бы извлечь эту информацию и рассчитать среднюю ежедневную эффективность для каждого оборудования, чтобы помочь в предиктивном обслуживании и оптимизировать производственный процесс.
...
![Пример использования узла html](../_static/U-Logic/Nodes/Parsers/html/html_example.png)

##### Документация по узлу

Выполняет преобразование между строкой JSON и ее представлением объекта JavaScript в любом направлении.

###### Входные значения

- `payload`: Объект JavaScript или строка JSON.
- `schema`: Необязательный объект JSON Schema для проверки payload. Свойство будет удалено перед отправкой `msg` на следующий узел.

###### Выходные значения

- `payload`:
  - Если входные данные представляют собой строку JSON, он пытается преобразовать ее в объект JavaScript.
  - Если входные данные — объект JavaScript, создается строка JSON. Строка может быть, по желанию, хорошо отформатирована.
- `schemaError`: Если проверка схемы JSON не пройдена, узел catch будет иметь schemaErrorсвойство, содержащее массив ошибок.

###### Подробности

По умолчанию узел работает с `msg.payload`, но его можно настроить для преобразования любого свойства сообщения.

Узел также может быть настроен на обеспечение определенной кодировки вместо переключения между ними. Это может быть использовано, например, с узлом http in, чтобы гарантировать, что payload является проанализированным объектом.

Если узел настроен на обеспечение кодирования свойства как строки и он получает строку, дальнейшие проверки свойства не будут выполняться. Он не будет проверять, является ли строка допустимым JSON, и не будет переформатировать ее, если выбрана опция форматирования.

#### xml

##### Документация по узлу

Выполняет преобразование между строкой XML и ее представлением объекта JavaScript в любом направлении.

###### Входные значения

- `payload`: Объект JavaScript или строка XML.

###### Выходные значения

- `payload`:
  - Если входные данные представляют собой строку, он пытается проанализировать ее как XML и создает объект JavaScript.
  - Если входные данные представляют собой объект JavaScript, он пытается построить XML-строку.

###### Подробности

При конвертации между XML и объектом любые атрибуты XML добавляются как свойство с именем $по умолчанию. Любое текстовое содержимое добавляется как свойство с именем _. Эти имена свойств можно указать в конфигурации узла.

Например, следующий XML будет преобразован, как показано:
...
Выполняет преобразование между строкой в ​​формате YAML и ее представлением объекта JavaScript в любом направлении.

###### Входные значения

- `payload`: Объект JavaScript или строка YAML.

###### Выходные значения

- `payload`:
  - Если входные данные представляют собой строку YAML, он пытается преобразовать ее в объект JavaScript.
  - Если входные данные представляют собой объект JavaScript, создается строка YAML.

### хранилище

#### write file

##### Что такое узел write file в U-Logic?

Узел write file в U-Logic используется для записи данных в файл в файловой системе. Он обычно используется в потоках, где вам нужно сохранить данные или журналы для последующего анализа или хранения. Содержимое, которое должно быть записано, указывается с помощью `msg.payload`.

##### Настройка узла write file в U-Logic:

- Имя файла: Укажите имя файла
  - path: Укажите путь к файлу, в котором будут сохранены данные.
  - msg.: Используйте свойство сообщения для динамической установки имени файла. По умолчанию будет использоваться `msg.filename`, а если `msg.filename` используется, файл будет закрыт после каждой записи. Для лучшей производительности используйте фиксированное имя файла.
  - выражение: используйте выражение JSON для динамической установки имени файла на основе данных в потоке.
  <!-- - Переменная среды (env var): используйте переменную среды для динамического задания имени файла. -->

- Действие: Выберите действие
  - добавить в файл: добавляет новые данные в существующий файл.
  - перезаписать файл: заменяет содержимое файла новыми данными.
  - удалить файл: удаляет указанный файл из файловой системы.

- Добавлять новую строку (\n) к данным?: включение этой опции добавит символ новой строки к каждому `payload` перед записью в файл.
- Создать каталог, если он не существует?: включение этой опции приведет к созданию указанного каталога, если его еще нет в файловой системе.
- Кодировка: указывает кодировку символов, которая будет использоваться при записи данных в файл. При выборе «set by msg.encoding» ее можно задать динамически.

*Совет: всегда используйте абсолютный путь для имени файла, чтобы U-Logic мог точно найти и обработать указанный файл.*

##### Варианты использования

1. **Регистрация данных**: сохранение показаний датчиков, данных устройств IoT или системных показателей в файле для исторического анализа или мониторинга тенденций с течением времени.
2. **Регистрация ошибок**: сбор и регистрация сообщений об ошибках, исключений или отладочной информации в файле для устранения неполадок и отладки.
3. **Вводимые пользователем данные**: сохраняйте вводимые пользователем данные или отправленные формы в файле, например, пользовательские настройки, отзывы или созданный пользователем контент.
4. **Отчетность**: создание отчетов в форматах CSV, JSON или обычного текста и сохранение их в файле для последующего извлечения или распространения.
5. **Резервное копирование и восстановление**: создание резервных копий критически важных данных или состояний системы для целей аварийного восстановления или контроля версий.
6. **Интеграция с внешними системами**: сохранение данных, полученных из API, баз данных или внешних служб, в файл для дальнейшей обработки или анализа.
7. **Архивирование**: архивируйте файлы журналов, исторические данные или устаревшие документы для сохранения записей о прошлых событиях или изменениях.
8. **Преобразование данных**: выполнение операций преобразования данных и запись преобразованных данных в файл в другом формате или структуре.
9. **Ведение журнала событий**: регистрация событий, уведомлений или взаимодействий пользователей в файле для целей аудита, соответствия или отслеживания истории.

##### Примеры

1. В приведенном ниже примере мы демонстрируем, как создать файл и записать в него содержимое.

![Пример настройки узла write file](../_static/U-Logic/Nodes/Storage/write_file/write_file_edit.png)

![Пример использования узла write file](../_static/U-Logic/Nodes/Storage/write_file/write_file_example1.png)

2. В приведенном ниже примере мы демонстрируем, как удалить файл с помощью узла write file

![Пример настройки узла write file](../_static/U-Logic/Nodes/Storage/write_file/write_file_edit2.png)

![Пример использования узла write file](../_static/U-Logic/Nodes/Storage/write_file/write_file_example2.png)

##### Документация по узлу

Записывает `msg.payload` в файл, добавляя в конец или заменяя существующее содержимое. Также может удалить файл.

###### Входные значения

- `filename`: Имя файла для обновления может быть указано в конфигурации узла или как свойство сообщения. По умолчанию будет использоваться `msg.filename` но это можно настроить в узле.
- `encoding`: Если кодировка настроена на установку с помощью msg, то это необязательное свойство может задать кодировку.
...
Каждое сообщение будет добавлено в конец файла, при желании между ними может быть добавлен символ новой строки (\n).
...
Его можно настроить на перезапись всего файла, а не на добавление. Например, при записи двоичных данных в файл, например, изображения, следует использовать эту опцию, а опцию добавления новой строки следует отключить.

Кодировку данных, записываемых в файл, можно указать из списка кодировок.

В качестве альтернативы этот узел можно настроить на удаление файла.

#### read file

##### Что такое узел read file в U-Logic?

Узел read file в U-Logic используется для чтения содержимого файла из файловой системы. Узел read file может читать как строковые, так и двоичные данные буфера, что делает его универсальным для различных потребностей интеграции в потоках U-Logic.

##### Настройка узла read file

- Имя файла: Укажите имя файла.
  - path: Определите путь к файлу, который необходимо прочитать.
  - msg.: Используйте свойство сообщения для динамической установки имени файла. По умолчанию будет использоваться `msg.filename`. Если `msg.filename` используется , файл будет закрыт после каждой записи. Для оптимальной производительности рассмотрите возможность использования фиксированного имени файла.
  - выражение: используйте выражение JSON для динамической установки имени файла на основе данных в потоке.
  <!-- - Переменная среды (env var): используйте переменную среды для динамического задания имени файла. -->

- Выход:
  - Одна utf-8 строка: Вывести содержимое файла как одну строку UTF-8.
  - сообщение для каждой строчки: Вывести каждую строку файла как отдельное сообщение.
  - один объект буфера: Вывести содержимое файла как один объект буфера.
  - поток буферов: Вывод содержимого файла в виде фрагментов буферов.

- Кодировка: укажите формат кодировки, который будет использоваться при чтении файла, если выходной формат — строка.

*Совет: всегда используйте абсолютный путь для имени файла, чтобы U-Logic мог точно находить и обрабатывать указанный файл.*

##### Варианты использования

1. **Загрузка конфигурации**: чтение файлов конфигурации, содержащих параметры или настройки для ваших потоков или приложений U-Logic. Это позволяет вам динамически настраивать поведение ваших потоков, не изменяя структуру потока.
2. **Агрегация данных**: чтение нескольких файлов данных и объединение их содержимого в одно сообщение или набор данных для дальнейшей обработки или анализа. Это может быть полезно для таких задач, как объединение нескольких файлов CSV в единый набор данных.
3. **Мониторинг системы**: читайте файлы системного журнала или отчеты о состоянии для мониторинга работоспособности и производительности различных компонентов вашей системы. Затем вы можете проанализировать эти данные в U-Logic для запуска оповещений или проведения диагностики.
4. **Анализ содержимого**: чтение файлов, содержащих структурированные форматы данных, такие как XML или JSON, и анализ их содержимого в U-Logic для извлечения релевантной информации. Это может быть полезно для таких задач, как извлечение данных из ответов API или анализ файлов конфигурации.
5. **Преобразование файло**в: чтение файлов в одном формате и преобразование их содержимого в другой формат с использованием возможностей обработки U-Logic. Например, вы можете прочитать файл CSV и преобразовать его содержимое в формат JSON для дальнейшей обработки или визуализации.
6. **Внешняя интеграция**: считывание данных из файлов, сгенерированных внешними системами или службами, и интеграция этих данных в ваши потоки U-Logic для дальнейшей обработки или действия. Это может облегчить взаимодействие между различными системами или приложениями.
7. **Мониторинг файлов**: Непрерывный мониторинг файлов на предмет изменений или обновлений с помощью узла Read File, запускающий потоки или действия на основе обнаруженных изменений. Это позволяет обрабатывать в реальном времени события, связанные с файлами, в вашем приложении U-Logic.
8. **Резервное копирование и восстановление данных**: читайте файлы, содержащие данные резервных копий или моментальные снимки, и используйте U-Logic для управления и автоматизации процессов резервного копирования и восстановления.

##### Примеры

1. В примере потока мы демонстрируем, как прочитать содержимое файла с помощью узла read file и получить вывод в виде строки.

![Пример настройки узла read file](../_static/U-Logic/Nodes/Storage/read_file/read_file_edit.png)

![Пример использования узла read file](../_static/U-Logic/Nodes/Storage/read_file/read_file_example.png)

2. В примере потока мы демонстрируем, как прочитать содержимое файла с помощью узла read file и получить вывод в виде буферного объекта.

![Пример настройки узла read file](../_static/U-Logic/Nodes/Storage/read_file/read_file_edit2.png)

![Пример использования узла read file](../_static/U-Logic/Nodes/Storage/read_file/read_file_example2.png)

##### Документация по узлу

Отслеживает изменения в каталоге или файле.

Вы можете ввести список каталогов и/или файлов, разделенных запятыми. Вам нужно будет поместить кавычки "..." вокруг всех, где есть пробелы.

Полное имя файла, который фактически изменился, помещается в `msg.payload` и `msg.filename`, а строковая версия списка наблюдения возвращается в `msg.topic`.

`msg.file` содержит только короткое имя файла, который был изменен. `msg.type` - тип измененного объекта, обычно файл или каталог , а `msg.size` размер файла в байтах.

```{note}
Каталог или файл должны существовать, чтобы за ними можно было наблюдать. Если файл или каталог удаляется, он может перестать отслеживаться, даже если будет создан заново.
``` 

###### Выходные значения

- `payload`: содержимое файла в виде строки или двоичного буфера.
- `filename`: если не настроено в узле, это необязательное свойство задает имя файла для чтения.
...
##### Описание
Узел api-helper предназначен для упрощения управления состоянием и навигацией между элементами пользовательского интерфейса. Он позволяет одновременно изменять состояния нескольких объектов, таких как кнопки, слайдеры, списки и фреймы. Узел поддерживает масштабирование значений для слайдеров.

##### Основные параметры узла

- **Имя узла (name)** — произвольное имя узла.
- **Имя интерфейса (ui)** — название интерфейса.
- **Тип сообщения (action_type)** — определяет тип события, которое узел отправляет. Поддерживаются два типа:
  - `"navigation"` — используется для навигации по интерфейсу.
  - `"uiState"` — используется для изменения состояния элементов интерфейса, таких как текст, видимость или цвет.

- **Объекты (object)** — элементы интерфейса с параметрами:
  - `name` — имя объекта, идентифицирующее элемент интерфейса.
  - `event` — тип события для объекта. Поддерживаемые события:
    - `uiState` — для изменения состояния (только кнопки).
    - `text` — для установки текста.
    - `value` — для числового значения (только слайдер).
    - `overlay` — для overlay.
    - `list item` — добавляет ключ и значение в список.
    - `remove list item` — удаляет элемент списка по ключу.
    - `list` — создает список или очищает его.
    - `x, y` — для смещения объекта по координатам.
    - `hidden` — для управления видимостью.
    - `fontColor` — для изменения цвета шрифта.
    - `value` — значение для слайдера.

  Для слайдера поддерживаются дополнительные параметры:
  - `min` и `max` — минимальное и максимальное входное значение, определяющие диапазон, который будет масштабироваться в диапазон от 0 до 1.
  - `frame` — для объектов типа overlay, указывающий имя frame, где будет отображаться overlay.
  - `list` — для объектов типа listbox, указывает список, состоящий из ключей и значений (key и value).

##### Работа узла

- **Подготовка данных:** Для изменения значений элементов отправьте JSON-объект в `msg.topic`. Каждый ключ в `msg.topic` должен соответствовать имени одного из объектов, указанных в параметре `object` узла. Значения зависят от типа объекта и аналогичны значениям, доступным по умолчанию в узле.
- **Тип события:** В зависимости от параметра `action_type` узел выполняет одно из двух действий:
  - `navigation`: создаёт объект для навигации по интерфейсу, ссылающийся на указанные элементы.
  - `uiState`: изменяет состояние указанных элементов интерфейса на основе переданных данных.

##### Пример использования

Предположим, у нас есть три элемента интерфейса: `LEVEL1`, `LEVEL2`, и `LEVEL3`, которые нужно обновить.

1. Настройте массив объектов в узле следующим образом:

```javascript
object = [
    { name: "button1", event: "uiState", value: "default" },
    { name: "LEVEL2", event: "value", min: 0, max: 1000 },
    { name: "LEVEL3", event: "value", min: 1000, max: 2000 }
];
```

2. Отправьте входное сообщение с данными, например:
...
msg.topic = {
    ...
    "LEVEL2": 500,
    "LEVEL3": 1500
    "List Box": [{"Key 1": "Value 1"}, {"Key 2": "Value 2"}]
};
```

3. Узел преобразует входные значения, которые могут выглядеть следующим образом:
...
msg.payload = {
    ...
    action_type: "uiState",
    object: {
        "button1": { event: "uiState", value: "selected" },
        "LEVEL2": { event: "value", value: 0.5 },
        "LEVEL3": { event: "value", value: 0.5 }
    }
};
```
...
Узел interlock предназначен для управления состояниями и видимостью объектов на пользовательском интерфейсе. Этот узел позволяет динамически обновлять состояние нескольких кнопок на основе входящих сообщений.

##### Использование

###### Настройка узла

При добавлении узла interlock в ваш поток вам нужно будет настроить его, используя следующие поля:

- **Имя узла:** Произвольное имя узла для его идентификации в потоке.
- **Имя интерфейса:** Имя интерфейса, которому будет отправлено сообщение.
- **Тип события:** Определяет тип события для кнопок. Может быть либо `uiState`, либо `hidden`.
- **Активное состояние кнопки:** Состояние, которое будет применено к активной кнопке.
- **Состояние кнопки по умолчанию:** Состояние, которое будет применено ко всем другим кнопкам.
- **Новые кнопки:** Поле для ввода имени новой кнопки и кнопка для добавления этой кнопки в список.
- **Список кнопок:** Выпадающий список добавленных кнопок с возможностью удаления выбранной кнопки.

###### Пример конфигурации

- **Имя узла:** My Interlock Node
- **Имя интерфейса:** MyInterface
- **Тип события:** uiState
- **Активное состояние кнопки:** selected
- **Состояние кнопки по умолчанию:** default
- **Добавление кнопок:** Введите имя кнопки в поле и нажмите Add, чтобы добавить кнопку в список. Повторите для каждой кнопки.

###### Формат входящих сообщений

Узел принимает имя активной кнопки через `msg.payload`. Например, если `msg.payload` содержит имя кнопки `Button1`, то это имя будет использовано для определения активного состояния кнопки. Для перевода всех объектов в состояние по умолчанию отправьте `reset state` в `msg.payload`.

###### Формат выходящих сообщений

Сообщение, отправляемое узлом, будет в следующем формате:

```json
{
    "UI": "Имя интерфейса",
    "action_type": "Тип действия (uiState или hidden)",
    "object": {
        "Имя кнопки 1": {
            "event": "Тип события",
            "value": "Состояние"
...
Если у вас есть три кнопки: `Button1`, `Button2` и `Button3`, и вы хотите установить состояние `selected` для `Button1`, а состояние `default` для остальных, вам нужно настроить узел следующим образом:

- **Имя интерфейса:** MyInterface
- **Тип события:** uiState
- **Активное состояние кнопки:** selected
- **Состояние кнопки по умолчанию:** default
- **Добавленные кнопки:** Button1, Button2, Button3

Отправьте сообщение с `msg.payload`, содержащим `Button1`, чтобы установить `Button1` в состояние `selected`, а остальные кнопки в состояние `default`.

###### Пример выходящего сообщения
...
Формат сообщений от интерфейса — JSON. Сообщение находится в `msg.payload`.

При каждом подключении или смене интерфейса приходит сообщение о выборе интерфейса.
...
{
    "select UI":"123",
    "Client":"192.168.66.14"
}
```

Каждое сообщение может содержать несколько ключей:

- **"Client"** – ключ, значение которого — IP-адрес клиента, инициатора события.
- **"UI"** – ключ, значение которого — имя интерфейса, на котором произошло событие. Имя интерфейса также является ключом, значением которого является JSON с информацией о событии.
- **"object"** – ключ, значением которого является имя объекта события.
- **"event"** – ключ, значением которого является тип события.
- **"value"** – ключ, значением которого является значение слайдера.

##### Значения для **"event"**:

- **"tap"** – кратковременное нажатие на кнопку.
- **"release"** – кнопка отпущена после нажатия.
- **"push"** – кнопка нажата.
- **"slide_begin"** – начало движения слайдера. Применимо только к слайдеру. В этом случае появляется ключ **"value"**, который содержит значение слайдера в формате `float`, но тип значения для ключа **"value"** — `String`.
- **"slide_end"** – слайдер отпущен. Применимо только к слайдеру. В этом случае появляется ключ **"value"**, который содержит значение слайдера в формате `float`.
- **"item_selected"** – нажатие на элемент списка. Применимо только к спискам. В этом случае появляется ключ **"value"**, значением которого является ключ элемента, который был указан при формировании списка.
...
формат сообщений для интерфейса - JSON. Сообщение должно находится в `msg.payload`
...
**"UI"**
Для объекта "UI" значением является имя интерфейса, которое было указано в редакторе интерфейса при создании интерфейса. Объект "UI" определяет какому интерфейсу предназначено сообщение

**"action_type"**
Для объекта "action_type" есть значение "navigation" и "uiState". Значение "navigation" является переключателем страницы. Т.е. для переключения страницы на интерфейсе необходимо использовать конструкцию в виде "action_type":"navigation". Значение "uiState" указывается при взаимодействии с элементами интерфейса, кнопки, слайдеры и т.д.

**"object"**
Значением объекта "object" является JSON. Так как внутри object находятся JSON, это позволяет одним сообщением изменить состояния нескольких объектов одного интерфейса(кнопок, слайдеров, текста и т.д.)

**"action_type":"uiState"**
"Имя объекта" - имя объекта (кнопки, слайдера, лейбла, списка, оверлея) задается в редакторе интерфейса. Так как взаимодействие происходит с помощью имени объекта, имена для каждого объекта должны быть уникальными. Имя объекта это уникальный индетификатор, который не должен повторяется внутри одного интрефейса.

**"event"** - Тип события, для объекта:

- "event":"uiState" - указатель на событие "состояние". Применим только к кнопкам
- "event":"value" - Указатель на событие "изменить значение слайдера. Применим только к слайдеру"
- "event":"list" - Указатель на событие "изменить список". Примени только к спискам
- "event":"text" - указатель на событие "изменить текст"
- "event":"fontColor" - Указатель на событие "Изменить цвет текста"
- "event":"url" - указатель на событие "изменить URL". Применим только к Web URL
- "event":"overlay" - Указатель на событие "переключить (отобразить оверлей)"
- "event":"hidden" - указатель на событие "скрыть/показать объект".
- "event":"x" - Указатель на событие "переместить объект по оси X"
- "event":"y" - Указатель на событие "переместить объект по оси Y"
- "event":"list" - Указатель на событие "список"

**"value"** - Значение/состояние объекта

Значение для объектов Button - "event":"uiState"
- "value":"deаfault" - переключить объект в состояние default
- "value":"selected" - переключить объект в состояние selected
- "value":"pressed" - переключить объект в состояние pressed
- "value":"disabled" - переключить объект в состояние disabled

**Пример изменения состояния одновременно двух кнопок**
...
**Slider** - "event":"value"
"event":"value" служит для изменеия значения слайдера. Slider принимает значения от 0 до 1. Значения в формате float "value":0.2 - изменит значение объекта Slider.
...
**list** - "event":"list_items"
"event":"list_items" служит для добавления или обновления ключа в списке. Обратите внимание "event":"list_items", имеет объект key и объект value. key - ключ, отображается в левой части списка, он же будет возврщаться при нажатии на этот элемент списка. value - отображается в правой части списка. Этот мотод не позволяет одновременно изменять или добавлять несколько элементов списка.

###### Пример сообщения для списка
...
list - "event":"list"
Это событие используется для формирования нового списка или очистки уже существующего. При каждом новом вызове события "event":"list" предыдущий список перезаписывается новым значением. Значение поля "value" должно быть массивом, в котором каждый элемент представляет собой отдельный JSON-объект (словарь). Если в качестве значения "value" указан пустой массив ([]), то это приводит к очистке списка.

**Пример 1: Формирование списка**
Здесь формируется новый список, состоящий из двух элементов. Каждый элемент — отдельный JSON-объект с ключами и значениями
...
```json
{
    ...
    "object": {
        "List Name": {
            "event": "list",
            "value": [{"key 1": "value 2"}, {"key 2": "value 2"}]
        }
    }
}
```

**Пример 2: Очистка списка**

В этом примере значение поля "value" — пустой массив, что приводит к очистке списка.
...
list - "event":"remove_list_items"
Это событие используется для удаления элемента списка по ключу. Поле "key" задает ключ удаляемого элемента. При обработке события система ищет элемент с указанным ключом в списке и удаляет его, если он существует
...
##### text
**text** - `"event":"text"`
- `"event":"text"` служит для передачи текста в текстовое поле объекта.
...
- "event":"fontColor" служит для изменения цвета текста RGB.
...
url - "event":"url"
"event":"url"служит для передачи url адреса объекту WEB URL.
...
"event":"overlay"служит для переключения overlay внутри определенного frame. Взаимодействие с Frame и Overlay осуществляется так же по имени которые указанно в графическом конфигураторе. Имя Frame указывается в ключе "frame"
...
hidden - "event":"hidden"
"event":"hidden" служит для скрытия или отображения скрытых объектов.
...
x - "event":"x"
"event":"x" служит для перемещения объекта по оси X. Ось x считается от левого края экрана до левого края объекта. Значения принимются в формате float от 0 и до разрешения экрана в проекте. Если разрешение экрана в проекте 1920x1080, Начало оси x = 0, конец = 1.92. Не перемещайте объект за границы разрешения проекта.
...
y - "event":"y"
"event":"y" служит для перемещения объекта по оси Y. Ось y считается от верзнего края экрана до верхнего края объекта. Значения принимются в формате float от 0 и до разрешения экрана в проекте. Если разрешение экрана в проекте 1920x1080, Начало оси y = 0, конец = 1.08. Не перемещайте объект за границы разрешения проекта.
