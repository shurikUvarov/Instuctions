...
Палитра узлов — это левая боковая панель, которая содержит все доступные узлы, включая основные узлы и установленные сторонние узлы.
...
<!-- могут ли сторонние узлы быть установлены -->
Палитра узлов разделена на несколько категорий, каждая из которых содержит коллекции узлов. При установке стороннего узла может быть создана новая категория. Подпотоки категоризируются в разделе «Подпотоки».
...
- Выполнив эти шаги, узел json преобразует входящую строку JSON в пригодный для использования объект JavaScript в потоке U-Logic. Более подробную информацию см. в документации по узлу [json]().
...
- **Отладка**: документация по отладке описывает методы и инструменты для выявления и устранения ошибок в ваших потоках U-Logic. Она объясняет, как использовать узел debug для отслеживания сообщений и переменных, а также предоставляет советы по интерпретации логов и диагностике проблем.
...
<!-- debugger - это устанавливаемый в палитре модуль -->
<!-- #### Отладка

Когда дело доходит до отладки потоков приложений в U-Logic, инструмент, к которому большинство разработчиков U-Logic часто обращаются, — это узел debug . Он предоставляет простой способ вывода полезных данных сообщений или других данных на боковую панель отладки, помогая вам получить представление о том, как работает ваш поток. Но что, если вам нужно больше контроля и видимости выполнения потока? Что, если вы хотите подробно пройти по каждому узлу, проверить переменные или приостановить поток в определенных точках, чтобы понять, что происходит?

В этих случаях использование отладчика U-Logic становится бесценным. Отладчик позволяет вам отслеживать выполнение ваших потоков в интерактивном режиме, устанавливать точки останова и получать более глубокие знания, выходящие за рамки того, что предлагает узел debug. Эта документация покажет вам, как эффективно использовать отладчик U-Logic для выявления проблем и тонкой настройки ваших приложений.

##### Что такое отладка и почему она так важна в потоках U-Logic?

Отладка — это поиск и исправление проблем в вашем коде или рабочем процессе. В U-Logic отладка помогает вам понять, как функционируют ваши потоки, предоставляя информацию об обрабатываемых данных и определяя, где что-то может пойти не так.

Обычно разработчики используют узел debug для вывода полезных данных сообщений и просмотра их на боковой панели. Хотя это полезно для простой отладки, это может быть ограничивающим, когда вам нужно устранить неполадки в более сложных сценариях. Поскольку потоки становятся больше и более взаимосвязанными, точное определение источника проблемы с использованием только узла debug может быть похоже на поиск иголки в стоге сена.

Вот тут-то и вступает в дело U-Logic Debugger , предлагая более детальный подход к отладке. Отладчик позволяет: -->
...
<!--- раздел про пользовательский узел, который необходимо скачать из менеджера палитры (U-Logic-contrib-loop) -->
...
<!-- сторонние дополнения из менеджера палитры? -->
Узел — это фундаментальный строительный блок, используемый для создания потоков. Каждый узел представляет собой отдельную часть функциональности или определенное действие, которое может быть выполнено в потоке. Эти узлы могут быть сторонними дополнениями с использованием менеджера палитры или основных узлов.
...
<!-- установочные узлы?  -->
Основные узлы — это набор узлов, которые включены в среду выполнения U-Logic по умолчанию, без необходимости процедуры установки узла. Для получения дополнительной информации о основных узлах обратитесь к части по основным узлам.
...
Палитра представляет собой боковую панель, содержащую все установленные и доступные для использования узлы.

![Palette Node](../_static/U-Logic/Terminology/palette_node.png)

### Менеджер палитры

<!-- устанавливаемые узлы? -->
Palette Manager в U-Logic — это инструмент, позволяющий пользователям управлять узлами, доступными для использования в их экземпляре U-Logic. Он предоставляет графический интерфейс для поиска, установки сторонних узлов с помощью Node Package Manager из палитры. Кроме того, он показывает все установленные узлы и позволяет пользователям обновлять и удалять их при необходимости. Установленные узлы автоматически добавляются в палитре узлов для легкого доступа и использования в потоках.

<!-- в палитре видно не то название -->

### Менеджер пакетов узлов (npm)

`npm` — это инструмент командной строки, используемый для управления дополнительными узлами и их зависимостями в U-Logic. Он позволяет пользователям устанавливать, обновлять и удалять узлы, которые вносятся или добавляются членами сообщества U-Logic. Он предоставляется Node.JS, средой выполнения для U-Logic.
...
<!-- Для получения дополнительной информации см. Импорт, экспорт и группировка потоков. -->
<!-- пользовательские узлы, что с ними?  -->
